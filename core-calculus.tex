...
Precision (Figure~\ref{fig:precision}) is a partial order that organises terms by \textit{information content}: for types, $\tau' \sqsubseteq \tau$ means $\tau$ has at least as much type information as $\tau'$. Here, $\gap$ is the global minimum. Expression precision is defined analogously; for instance, the slices of $\lambda x \mathbin{:} 1.\; x$ form the precision chain $\gap \sqsubseteq \lambda x \mathbin{:} \gap.\; \gap \sqsubseteq \lambda x \mathbin{:} 1.\; \gap \sqsubseteq \lambda x \mathbin{:} 1.\; x$, each step filling in one more part of the term.
