...
\vspace{0pt}
A gradually-typed language elaborates source programs into a cast calculus and runs the casts dynamically. Type slices can be attached directly to those casts, letting casts explain why they were inserted by pointing back to code: the source is a slice of the term being cast, the target a slice of its context.
...
This paper formulated type slicing theory for a bidirectional calculus with explicit System~F polymorphism, bindings, products, and sums, mechanised in Agda, and formally integrated it with the error-marking calculus of \citet{Marking2024}, applying type slicing to ill-typed programs and justifying its \textit{sound} use in debugging \textit{all} static type errors. This foundation positions type slicing as a promising tool for practical languages, especially gradually typed ones, explaining both static \textit{and, in follow-up work, dynamic} types.
