...
\begin{itemize}
\item \textbf{Function position} ($e_1$ is the focus, $\C = \cmark(e_2)$): The corresponding classification is: $\ctxclass{\cmark(e_2)}{\synmode\;\tau_2}{\Delta;\,\Gamma}{\synmode\;\tau}$. The focus must synthesise type $\tau$ as per the focus mode, with the extra premises being $\tau \sqcup (\gap \funarr \gap) \equiv \tau_1 \funarr \tau_2$ and $\analysis{e_2}{\tau_1}$.
\item \textbf{Argument position} ($e_2$ is the focus, $\C = e_1(\cmark)$): The corresponding classification is: $\ctxclass{e_1(\cmark)}{\synmode\;\tau_2}{\Delta;\,\Gamma}{\anamode\;\tau_1}$. The focus analyses against $\tau_1$ as per the focus mode, with the extra premises being: $\synthesis{e_1}{\tau}$ and $\tau \sqcup (\gap \funarr \gap) \equiv \tau_1 \funarr \tau_2$.
\end{itemize}
