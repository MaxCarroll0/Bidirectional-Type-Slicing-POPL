...
\begin{definition}
\label{def:ana-slice}
An \textit{analysis slice} of $\mathit{CD} : \ctxclass{\C}{{\color{BrickRed}\Uparrow}\,\tau_p}{\Delta';\,\Gamma'}{{\color{BlueViolet}\Downarrow}\,\tau}$ on $\upsilon \in \lfloor\tau\rfloor$, written $\anaslice{\mathit{CD}}{\upsilon}$, is a pair $(\kappa, \gamma) \in \lfloor\C, \Gamma\rfloor$ such that $\ctxclass[\Delta;\,\gamma]{\kappa}{{\color{BrickRed}\Uparrow}\,\psi}{\Delta'';\,\Gamma''}{{\color{BlueViolet}\Downarrow}\,\upsilon}$ for some outer-type witness $\psi \sqsupseteq \upsilon$ (with $\psi \in \lfloor\tau_p\rfloor$ by graduality, \cref{thm:graduality-syn-cls}).
\end{definition}
...
$\minanaslice{\mathit{Cls}}{\upsilon}$ is defined analogously to Definition~\ref{def:isminimal}, minimising over the context slice $(\kappa, \gamma)$. And as before, existance of minimal analysis slices and monotonicity follow from finiteness and graduality.
...
\noindent As mentioned earlier, a synthesising type derives all of its type information internally.\footnote{Again, given that assumptions are tracked externally.}. Therefore, the type is actually only enforced by the context up until the innermost subderivation with a synthesis outer derivation mode. 

For example, consider an annotated term inside an annotation, only the inner annotation actually enforces a type of the inner term. Therefore, an algorithm need only check this region (the outer context can be fully omitted). Further, there is little point in highlighting the outer context in practice as it has no influence on the types; this is the approach the Hazel implementation takes, to avoid excessive highlighing.
