...
\[ \Gamma = (x \mathbin{:} \mathbf{*} \funarr \mathbf{*},\; y \mathbin{:} \mathbf{*} \funarr \mathbf{*}), \qquad e = \mathbf{if}\;\mathbf{true}\;\mathbf{then}\;x\;\mathbf{else}\;y, \qquad \tau = \mathbf{*} \funarr \mathbf{*}, \]
and ask for slices of $D : \synthesis{e}{\tau}$ at the full query $\upsilon = \mathbf{*} \funarr \mathbf{*}$. There are \textit{four} incomparable minimal full slices, displayed together as underlines:
...
\begin{tikzpicture}[baseline=(t.base), every node/.style={inner sep=3pt}]
  \node[draw, line width=0.9pt, rounded corners,
        ...
        {$x : \sAC{\mathbf{*}} \mathrel{\sABC{\funarr}} \sBC{\mathbf{*}},\quad y : \sBD{\mathbf{*}} \mathrel{\sABD{\funarr}} \sAD{\mathbf{*}}$};
...
\begin{description}
\item[(A) {\color{BrickRed}Red}]: Both branches are retained; the assumption slice keeps $x$'s domain only and $y$'s codomain only (joining to $* \funarr *$).
...
\begin{tikzpicture}[baseline=(t.base), every node/.style={inner sep=3pt}]
  \node[draw, line width=0.9pt, rounded corners,
        label={[font=\scriptsize\itshape]above:assumptions}] (a)
        {$x : \sPboth{\mathbf{*}\funarr\mathbf{*}},\quad y : \sPtwo{\mathbf{*}\funarr\mathbf{*}}$};
  \node[below=12pt of a] (t)
...
\begin{figure}[H]
\centering
$e \mathbin{\synmode} \forall\alpha.\,(\alpha \times \alpha \times \mathbf{Bool})$,\quad applied at\quad $\sigma = \mathbf{*} \funarr (\mathbf{Bool} \times \mathbf{Bool})$:

\medskip

So $e\langle\sigma\rangle \mathbin{\synmode} (\mathbf{*} \funarr (\mathbf{Bool} \times \mathbf{Bool})) \times (\mathbf{*} \funarr (\mathbf{Bool} \times \mathbf{Bool})) \times \mathbf{Bool}$.

\medskip

\begin{tabular}{r@{\;\;}l}
\textbf{Query}             & $\upsilon \;=\; \underbrace{\sli[teal]{\mathbf{*} \funarr \gap}}_{\alpha} \;\times\; \underbrace{\sli[teal]{\gap \funarr (\mathbf{Bool} \times \gap)}}_{\alpha} \;\times\; \gap$ \\[16pt]
\textbf{Matched query}     & $\widehat{\upsilon} \;=\; \alpha \;\times\; \alpha \;\times\; \gap$ \\[8pt]
\textbf{Constraints}       & $\mathbf{*} \funarr \gap \;\sqsubseteq\; \alpha \qquad \gap \funarr (\mathbf{Bool} \times \gap) \;\sqsubseteq\; \alpha$ \\[8pt]
\textbf{Constraints Join}  & $\hat\sigma \;=\; \mathbf{*} \funarr (\mathbf{Bool} \times \gap)$ \\
\end{tabular}
...
An \textit{inner analysis slice} of $\mathit{Cls} : \ctxclass{\C}{\anamode\,\tau_p}{\Delta';\,\Gamma'}{\anamode\,\tau}$ on $\upsilon \in \lfloor\tau\rfloor$, written $\anaposslice{\mathit{Cls}}{\upsilon}$, is a pair $(\kappa, \gamma) \in \lfloor\C, \Gamma\rfloor$ such that, for some chosen outer type $\upsilon_{\mathrm{outer}} \in \lfloor\tau_p\rfloor$, $\ctxclass[\Delta;\,\gamma]{\kappa}{\anamode\,\upsilon_{\mathrm{outer}}}{\Delta'';\,\Gamma''}{\anamode\,\upsilon}$.
\end{definition}
