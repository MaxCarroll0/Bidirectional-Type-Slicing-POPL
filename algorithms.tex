...
\begin{tikzpicture}[baseline=(t.base), every node/.style={inner sep=3pt}]
  \node[inner sep=0pt] (ax) {$x : \sAC{\mathbf{*}} \mathrel{\sABC{\funarr}} \sBC{\mathbf{*}}$};
  \node[inner sep=0pt, anchor=west] (xmark) at (ax.west) {$\phantom{x : }$};
  \node[inner sep=0pt, right=3.5em of ax] (ay) {$y : \sBD{\mathbf{*}} \mathrel{\sABD{\funarr}} \sAD{\mathbf{*}}$};
...
\begin{description}
\item[{\color{BrickRed}(A)}] Both branches are retained; the assumption slice keeps $x$'s domain only and $y$'s codomain only (joining to $* \funarr *$).
...
\begin{center}
\begin{tikzpicture}[baseline=(t.base), every node/.style={inner sep=3pt}]
  \node[inner sep=0pt] (ax) {$x : \sPboth{\mathbf{*}\funarr\mathbf{*}}$};
  \node[inner sep=0pt, anchor=west] (xmark) at (ax.west) {$\phantom{x : }$};
  \node[inner sep=0pt, right=2.5em of ax] (ay) {$y : \sPtwo{\mathbf{*}\funarr\mathbf{*}}$};
  \node[inner sep=0pt, anchor=west] (ymark) at (ay.west) {$\phantom{y : }$};
  \node[inner sep=0pt] at ($(ax.east)!0.3!(ay.west)$) {$,$};
  \node[draw, line width=0.9pt, rounded corners, fit=(ax)(ay),
        label={[font=\scriptsize\itshape]above:assumptions}] (a) {};
