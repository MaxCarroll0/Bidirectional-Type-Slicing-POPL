...
Then, if a synthesis slice is queried on this case tree with $\tau \times * \times \dots \times *$, each $I_j$ must be retained, so the whole tree structure must be retained, and the only variance in the size of the slices is purely dependent on which variables $x_j$ were retained.
