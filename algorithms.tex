...
As slices can be inexact, efficiently calculating a minimal context providing sufficient assumptions to a minimal \textit{SynSlice} at the focus is non-trivial. The Hazel implementation instead approximates this by slicing the definition / annotation on each binding by its required type as per the minimal assumptions of the focused term. When each slice of these definitions is exact,\footnote{Inexactness can only come from synthesising let bindings lacking annotations and with variable aliasing whilst querying the definition at a refined (not full) query. Therefore, we expect this approximation to be exact in most situations.} the resulting type slice is minimal.
