...
Consider $x \mathbin{:} \mathbf{*} \funarr \mathbf{*} \vdash (x, x) \synmode (* \funarr *) \times (* \funarr *)$, and query $\upsilon = (\mathbf{*} \funarr \gap) \times (\gap \funarr \mathbf{*})$. 

Any strictly smaller slice of either $x : * \funarr *$ or $(x, x)$ synthesises a type strictly less precise than $\upsilon$. Yet the original program synthesises $(* \funarr *) \times (* \funarr *)$, strictly more precise than $\upsilon$.
\end{counterexample}
...
\tikzset{
  ...
  slpath/.style={Green!60!black, opacity=0.32, line width=6pt,
...
\end{tikzpicture}
\caption{Monotonically descending minima $m_2, m_1, m_0$ for query refinements $\upsilon_2 \to \upsilon_1 \to \upsilon_0$, calculated by descending along the \textcolor{Green!60!black}{highlighted} path.}
...
\paragraph{Branch hopping.} This is where highlighted regions switch between branches when refining the query, requiring the user to switch their reasoning between the contents in each branch. For example, refining $\upsilon = \mathbf{*}\funarr\mathbf{*}$ to $\upsilon' = \gap\funarr\mathbf{*}$:
\begin{center}
\begin{tikzpicture}[baseline=(t.base), every node/.style={inner sep=3pt}]
  \node[draw, line width=0.9pt, rounded corners,
        label={[font=\scriptsize\itshape]above:assumptions}] (a) {$x : \sli{\mathbf{*} \funarr \mathbf{*}}$};
...
\begin{tikzpicture}[baseline=(t.base), every node/.style={inner sep=3pt}]
  \node[draw, line width=0.9pt, rounded corners,
        label={[font=\scriptsize\itshape]above:assumptions}] (a) {$y : \mathbf{*}\,\sli{\funarr \mathbf{*}}$};
