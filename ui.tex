...
{This section previews type slicing, formally developed in \cref{sec:synthesis,sec:analysis} over the core calculus of \cref{sec:core-calculus}, through two worked examples in the Hazel implementation. 
...
\caption{Refined Synthesis slice and analysis slices.}
...
\emph{Explain error} queries the slices by taking only the outermost inconsistency in the types (the function vs the option in the return type). The resulting slice collapses the program to include only the function shape of the continuation and the offending annotation.

Crucially, the slice did not assume the annotation is the ground truth. It shows the user why both the expectation's \emph{source} (the annotation) and the \emph{usage} (the function), leaving the programmer to judge which is wrong. This is justified as programmers often add annotations incorrectly to existing code (49\% of the time~\cite{TypeAnnotationStudy}).} Should the user instead wish to trust the context unilaterally, they can simply only consider the synthesis slice (omitting the analysis slice entirely). \Cref{sec:marking} makes slicing of type errors precise, applying slicing to marked ill-typed programs.

{The user interface is relatively primitive, only folding sliced-away code. Minor improvements could include: highlighting \textit{sources} of type information (e.g. annotations) in order to distinguish these from the `spine' of the program around the selected term (required only to make it syntactically well-formed), or extensions to folds to allow folding entire unused let expressions, or case branhces\footnote{As shown in figure 1.}. Additionally, more radical changes could show the minimal types required for each variable used in slice, combined with features like jump-to-definition to quickly navigate within the slice.}
