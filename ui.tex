...
{This section previews type slicing through two worked examples in the Hazel implementation.
...
Crucially, the slice did not assume the annotation is the ground truth. It shows the user both the expectation's \emph{source} (the annotation) and its \emph{usage} (the function), leaving the programmer to judge which is wrong. This is justified as programmers often add annotations incorrectly to existing code (49\% of the time~\cite{TypeAnnotationStudy}).} A user who instead wishes to trust the context unilaterally can consider only the synthesis slice, omitting the analysis slice entirely. We make slicing of type errors precise later (\S\ref{sec:marking}).

{The user interface is relatively primitive, only folding sliced-away code. Minor improvements could include highlighting \textit{sources} of type information (e.g. annotations) to distinguish them from the program's `spine' (required only for syntactic well-formedness), or folding entire unused let expressions or case branches (as in \cref{fig:parse-digit-synthesis}). More radical changes could show the minimal types required for each variable in the slice, with jump-to-definition to navigate within it.}
