...
This section formally applies type slicing to static error squiggles as in \cref{fig:parse-digit-slices} (\cref{sec:explanation}). It demonstrates that we can not only use this calculus to ``explain types'' in well-typed programs, but also effectively ``explain type errors'' in ill-typed programs.

This builds upon the theory of type error marking for bidirectional systems with holes of Zhao et al.~\cite{Marking2024}, which we briefly reca nextp and extend to the type slicing core calculus (\cref{sec:core-calculus}).
...
\begin{enumerate}
\item Place the focus (cursor) at some marked term. This splits the marked program into a marked context $\chk C$ and marked focus $\chk e$, with $\chk C[\chk e]$ recovering the program; a marked context classification at the focus is then derivable by totality of marked context classification (supplementary material).
...
\item Pick queries to explore the error. For type-inconsistency marks \textit{both} the synthesis and analysis slices exist (two context classifications exist from step 1). From these,you can query \textit{only} the inconsistent parts. For shape errors, simply query just the outermost shape of the synthesised type; mode-limitation errors are analogous on the analysis side.
\end{enumerate}

I give some worked examples, notating with a \markbox{\text{marked term}}, a \sli{\text{synthesis slice}}, and/or an \sli[OliveGreen]{\text{analysis slice}}. `Purely structural' slice regions which don't add additional or manipulate type information (i.e. bindings) are in a lighter shade.
...
\]
The RHS $(\texttt{true},\,\texttt{false})$ synthesises $\texttt{Bool} \times \texttt{Bool}$ but is analysed against $* \Rightarrow *$. Marking inserts
...
\]
Marking would ordinarily reports an error that $\markincon{\texttt{Bool} \times \texttt{Bool}}{* \Rightarrow *}$ with the entire red term highlighted. However, in reality, only the outermost type constructors are actually causing the (initial) type inconsistency, and type slicing allows query refinements of the full types. In full:
...
\queryblock{\texttt{Bool}\sli{\times}\texttt{Bool}\;\;(\equiv\;\gap \times \gap)}{\texttt{*}\sli[OliveGreen]{\Rightarrow}\texttt{*}\;\;(\equiv\;\gap \Rightarrow \gap)}
\[
...
However, type slicing is flexible. A user who \text{does} want to trust the surrounding context (annotations), as is often done in conventional type error highlighting, can simply only query the synthesis-side. Or vice versa, to trust the implementation, which is useful when refactoring code to a different type, which conflicts with existing annotations.
...
A representative example is $e = \texttt{(*, *)}\,(\,\texttt{true}\,)$. Where $\texttt{(*,*)}$ is syntactically in a function position, but synthesises $\texttt{*}\times\texttt{*}$, which is \textit{not} a function, i.e. does not join with $\gap \Rightarrow \gap$. We can therefore query just why it is a product, and not a function.
...
determined by the surrounding context. Consequentially it has
no synthesis type, so type inconsistencies are not marked via subsumption (type inconsistency), but instead by a mode limitation mark. In this situation all the relevant type information is derived from the surrounding context, so we use analysis slices.
